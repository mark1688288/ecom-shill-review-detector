...
| KD-04 | 預設 adapter | **`fixture`**（JSONL replay）；`json_api` 為介面 + **零 HTTP stub** | **2026-08-27 使用者確認**：v1 fixture-first，不實作 live crawler / HTTP。未來若另有指令點名 marketplace 才開 PR。任何未來 live adapter 的 ToS 風險仍 **High**。Named exception：操作者明示的 `ecom-shill harvest`（Bright Data Browser API → FixtureReviewRaw JSONL；見 [`docs/design-bright-data-scrapping-pro-browser-hktvmall.md`](design-bright-data-scrapping-pro-browser-hktvmall.md)）。操作者明示的 `ecom-shill harvest --transport scrapingbee`（HTML API → JSONL；見 [`docs/design-scrapingbee-hktvmall-reviews.md`](design-scrapingbee-hktvmall-reviews.md)）同屬 named exception。CI、`crawl --adapter fixture`、`json_api` stub **仍零 live HTTP**。 |
...
5. 操作者明示的 `ecom-shill harvest --transport scrapingbee`（HTML API → JSONL）同屬 KD-04 named exception；CI／`fixture`／`json_api` **不變**。見 [`docs/design-scrapingbee-hktvmall-reviews.md`](design-scrapingbee-hktvmall-reviews.md)。
