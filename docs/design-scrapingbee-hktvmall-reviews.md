...
| KD-SB-07 | `n_declared_reviews` | 優先 `span.comment__count` 整數；fallback 既有 `HKTVMALL_DECLARED_REVIEWS_RE`（HTML 字串）。總頁優先 `span.total` `/共N頁`，**多個 span.total 取 max**（Q&A `共1頁` vs 評論 `共N頁`）。**忽略** JSON-LD／`metadata['json-ld']` | Probe：JSON-LD 可為 0，tab 顯示 25。first-match 會把 Q&A 1 頁當成評論總頁。 |
